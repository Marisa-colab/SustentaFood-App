...
// 1. Importação da interface LicenseInfo adicionada
import { WasteLog, SummaryMetrics, AlertItem, LicenseInfo } from '../types';
...
interface DashboardViewProps {
  ...
  license?: LicenseInfo; // 2. Prop da Licença adicionada
}
...
export const DashboardView: React.FC<DashboardViewProps> = ({
  ...
  onOpenNewWasteModal,
  setActiveTab,
  license // 3. Receber prop da licença
}) => {
  ...
  return (
    <div className="space-[#121212] space-y-6 pb-12">
      ...
      {/* 4. BLOCO EXECUTIVO DE LICENÇA E PLANO SAAS (Inserido no topo) */}
      {license && (
              ...
              <div>
                <div className="flex items-center gap-2">
                  <span className="text-[10px] font-bold uppercase tracking-wider text-emerald-400 bg-emerald-500/10 px-2 py-0.5 rounded border border-emerald-500/30">
                    Plano {license.planType || 'SaaS Comercial'}
                  </span>
                  <span className="text-xs text-slate-400">Subscrição Ativa</span>
                </div>
                <h3 className="text-base font-bold text-white mt-0.5">{license.clientName}</h3>
              </div>
            </div>

            <div className="flex items-center gap-6 text-xs text-slate-300">
              <div>
                <span className="block text-slate-400 text-[10px] uppercase font-medium">Chave de Licença</span>
                <span className="font-mono text-slate-200">{license.licenseKey}</span>
              </div>

              <div className="border-l border-slate-700 pl-6">
                <span className="block text-slate-400 text-[10px] uppercase font-medium">Validade da Subscrição</span>
      ...
      {/* SPECIAL MODULE: Simulador de Poupanças */}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        {/* Top 10 Wasted Products Bar Chart */}
